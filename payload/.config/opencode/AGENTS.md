...
- Bootstrap instructions live in `~/.config/opencode/prime-directive.md` and `~/.config/opencode/runtime/bootstrap.md`.
- Hidden runtime context is injected by `~/.config/opencode/plugins/autonomy-runtime.js`.
- Post-turn KB recommendations flow through `~/.config/opencode/plugins/kb-post-turn-analyzer.js` on compaction.
- File-based working memory lives under `~/.config/opencode/memory/` and supports explicit project-local `.opencode/memory/` state.
- Global installs keep traces, checkpoints, and eval helpers under `~/.config/opencode/{traces,state,evals}`.
